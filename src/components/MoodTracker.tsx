...
const moodOptions: MoodOption[] = [
  { rating: 1, label: 'Awful', color: 'bg-red-400 hover:bg-red-500' },
  { rating: 2, label: 'Bad', color: 'bg-orange-400 hover:bg-orange-500' },
  { rating: 3, label: 'Okay', color: 'bg-yellow-300 hover:bg-yellow-400' },
  { rating: 4, label: 'Good', color: 'bg-green-400 hover:bg-green-500' },
  { rating: 5, label: 'Amazing', color: 'bg-emerald-400 hover:bg-emerald-500' },
];
          ...
          {moodOptions.map((mood) => (
            <div key={mood.rating} className="flex flex-col items-center">
              <button
                ...
                className={`${mood.color} w-14 h-14 rounded-full flex items-center justify-center text-lg text-white font-bold transition-transform hover:scale-110 mb-2`}
              >
                {mood.rating}
              </button>
